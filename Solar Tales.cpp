...
// Функция для добавления планеты (обновлена для сохранения)
void addPlanet() {
    Planet planet;
...
// Функция для добавления спутника (обновлена для сохранения)
void addSatellite() {
    if (planets.empty()) {
...
// Функция для удаления данных (обновлена для сохранения)
void deleteData() {
    clearScreen();
    cout << "\" SOLAR TALES \"\n\n";
